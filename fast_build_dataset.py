...
def parse_answer_pdf(pdf_path):
    print(f"Extracting answers and solutions from {pdf_path}...")
    doc = fitz.open(pdf_path)
    total_pages = len(doc)
    solutions_dict = {}
    ...
    for i in range(total_pages):
        ...
        if current_q_num:
            ...
            sol_lines = lines[sol_idx+1:] if sol_idx != -1 else lines
            sol_text = "\n".join(sol_lines)
            ...
            if clean_exp and len(clean_exp) > len(solutions_dict[current_q_num]["explanation"]):
                solutions_dict[current_q_num]["explanation"] = clean_exp

        if (i + 1) % 30 == 0 or i == total_pages - 1:
...
def process_session(q_pdf, ans_pdf, output_json, topic_name):
    ...
    for q_num in range(1, 91):
        ...
        if "SS1" in q_pdf:
            ...
            elif 47 <= q_num <= 90: category = "Financial Statement Analysis"
        elif "SS2" in q_pdf:
            ...
            elif 81 <= q_num <= 90: category = "Portfolio Management"
        ...
        final_questions.append({
            ...
            "explanation": sol_data["explanation"] if sol_data["explanation"] else "Refer to detailed solution explanation in CFA curriculum."
        })
